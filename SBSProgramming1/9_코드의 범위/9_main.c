...
// 특정 데이터를 저장하는 공간 "메모리"
// 0 또는 1 이외는 컴퓨터는 알아 들을 수 없다

/*
* 변수, 함수
* 변수가 왜 필요합니까? 변수를 저장하는 자료형을 지정을 해줬다.
* 함수가 왜 필요합니까? 명령어를 작성 statement(명령문) 세미클론 명령이 끝났다는 표시
* 
* 개발을 할 때 한글로 먼저(Pseudo Code) 표현하라.
* 명령문은 한줄로 표현을 못한다. -> 코드를 해석하기도 힘들고, 반복적으로 사용하는 코드를 표현하기 힘들다.
*/
...
int main()
{
	...
	ModifyStatValue(STR); // Q. 어떤 문제 때문에 한번에 표현을 못합니다
	ModifyStatValue(DEX);
	ModifyStatValue(INT);
	ModifyStatValue(LUK);
...
// Q1. main에도 LUK, Func에도 LUK이 있다. 컴퓨터가 같은 이름을 구별을 할까? = 구별할 수 없기 때문에 따로 지정해야한다?
// Q1-2. 이름이 전혀 겹치지 않게 만들면 되는거 아닌가요? -> 이름이 같더라도 사용할 수 있게 만들 수 없을까? -> 지역을 표현.

// Q2. StatDevideFunc 함수를 만났을 때 Local이 변경이 되어서 StatDevideFunc의 변수를 표현하고 있다. 함수가 종료가 되었을 때 LUK 사라졌다.
// main() 'LUK = 1' Stat 'LUK = 3' 스택 / 힙 영역   //stack over flow
...
// 장점) 메모리 관리가 수월하게 컴퓨터 영역을 만들었구나. - 단점) 메모리에 저장해두었다가 사용할 수는 없을까? => 힙 영역 구조체(struct)


// 함수를 정의하는 단계
void StatDevideFunc() // 들여쓰기를 자동으로 하는 명령어. (ctrl + k) + (ctrl + d)
{
	// 가중치 플레이어의 레벨, 아이템 등등을 가졌을 때 추가적인 스탯을 준다. => 일시적으로 메로리를 저장해두었다가 지우고 싶다.

	int LUK;
	//scanf("%d", &LUK);
	//printf("num의 값 : %d\n", LUK);

	//StatDevideFunc(); // 디버깅 F11하면 무한루프한다
	// 코드가 끝날때 LUK에 할당되어 있는 메모리를 컴퓨터에서 비워줘 명령어
	// 반복문, 조건문, 함수 => 코드를 구현할 수 있다. 
	// 스탯 시스템. 포인트 -> STR, DEX, INT, LUK 선택해서 스탯을 부여한다. 
}	// <- 세미클론이 없다. for반복문 for(int i = 0; i< 10; i++); {  }}   
    // 중괄호가 곧 코드의 끝을 표현하기 때문이다.

int ReturnStatAmount(int baseAmount, int itemStatAmount)
{
	int result = 0; // 특정 목적을 위해 만든 변수 끝나면 사라짐
	// 게임에 레벨 (Easy, Normal, Hard) 
	int levelCount = 1;
	// if Easy 1, Normal 2, Hard 3
	int itemWeight = 10;
	// 장착한 아이템에 따른 스탯 
	// 랜덤으로 주사위 굴림을 통한 결과 값을 도출
	int stat = 10;
